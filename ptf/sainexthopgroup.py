...
def setup_hash(self):
    ...
    # create Lag hashes
    if not hasattr(self, 'sai_lag_available') or self.sai_lag_available:
...
class L3IPv4EcmpHostTwoPortsHelper(SaiHelperSimplified):
    """
    Base ECMP tests for IPv4 and ECMP members as regular L3 port RIFs
    Adapted for usage only of 2 ports
    """
    def setUp(self):

        super(L3IPv4EcmpHostTwoPortsHelper, self).setUp()
        ...
        save_number_of_available_nhg_resources(self, False)
        try:
            self.sai_lag_available = self.check_sai_object_type_availability(
                sai_obj_type=SAI_OBJECT_TYPE_LAG)
        except NameError as e:
            print(f"\nChecking SAI_OBJECT_TYPE_LAG availability...")
            print('Name Error:', e, 'and not available')
            self.sai_lag_available = False
        # set switch src mac address
        sai_thrift_set_switch_attribute(
            self.client, src_mac_address=ROUTER_MAC)
        sai_thrift_set_switch_attribute(
            self.client, ecmp_default_hash_seed=TEST_ECMP_SEED)
        if self.sai_lag_available:
            sai_thrift_set_switch_attribute(
                self.client, lag_default_hash_seed=TEST_LAG_SEED)
        self.create_routing_interfaces(ports=[0, 1])
        # test neighbor creation
        self.neighbor_entry1 = sai_thrift_neighbor_entry_t(
            self.switch_id, self.port0_rif, sai_ipaddress('10.10.10.1'))
        sai_thrift_create_neighbor_entry(
            self.client, self.neighbor_entry1, dst_mac_address=dmac1)
        self.neighbor_entry2 = sai_thrift_neighbor_entry_t(
            self.switch_id, self.port1_rif, sai_ipaddress('10.10.10.2'))
        sai_thrift_create_neighbor_entry(
            self.client, self.neighbor_entry2, dst_mac_address=dmac2)
        self.nhop1 = sai_thrift_create_next_hop(
            self.client,
    ...
    def tearDown(self):
        try:
            ...
            super(L3IPv4EcmpHostTwoPortsHelper, self).tearDown()


class L3SaiNhgSetGetTwoPortsTest(L3IPv4EcmpHostTwoPortsHelper):
    """
    Checks SAI switch ECMP attributes and validates
    get and set attributes
    """
    def setUp(self):
        super(L3SaiNhgSetGetTwoPortsTest, self).setUp()

    def runTest(self):
        print("L3SaiNhgSetGetTwoPortsTest")
        try:
            # predefined NHG self.nhop_group1 with 2 members
            nhg1_size = 2
            attr_list = sai_thrift_get_switch_attribute(
    ...
    def tearDown(self):
        super(L3SaiNhgSetGetTwoPortsTest, self).tearDown()


class L3IPv4EcmpHostTwoPortsTest(L3IPv4EcmpHostTwoPortsHelper):
    """
    IPv4 ECMP tests with all members which are port RIFs
    Adapted for usage only of 2 ports
    """
    def setUp(self):
        super(L3IPv4EcmpHostTwoPortsTest, self).setUp()
    ...
    def tearDown(self):
        super(L3IPv4EcmpHostTwoPortsTest, self).tearDown()


class L3IPv4EcmpRoutingTwoPortsTest(L3IPv4EcmpHostTwoPortsHelper):
    """
    IPv4 ECMP routing test with random host
    """
    def setUp(self):
        super(L3IPv4EcmpRoutingTwoPortsTest, self).setUp()

    def runTest(self):
        print("L3IPv4EcmpRoutingTwoPortsTest")
        for n_packet in range(4):
            host_ip = '10.10.{}.{}'.format(random.randint(2, 255), random.randint(3, 255))
            ...
            print("Sending packet port %d -> port [%d,%d]"
                "(192.168.0.1 -> %s [id = 101])" % (
                    self.dev_port0, self.dev_port0, self.dev_port1, host_ip))
            send_packet(self, self.dev_port0, pkt)
            verify_any_packet_any_port(
                self, [exp_pkt1, exp_pkt2], [self.dev_port0, self.dev_port1])

            print("Sending packet port %d -> port [%d,%d]"
                "(192.168.0.1 -> %s [id = 101])" % (
                    self.dev_port1, self.dev_port0, self.dev_port1, host_ip))
            send_packet(self, self.dev_port1, pkt)
            verify_any_packet_any_port(
                self, [exp_pkt1, exp_pkt2], [self.dev_port0, self.dev_port1])

    def tearDown(self):
        super(L3IPv4EcmpRoutingTwoPortsTest, self).tearDown()
...
class L3IPv6EcmpHostTwoPortsHelper(SaiHelperSimplified):
    """
    Basic ECMP tests for IPv6 and regular L3 port RIFs
    Adapted for usage only of 2 ports
    """
    def setUp(self):
        ...
        super(L3IPv6EcmpHostTwoPortsHelper, self).setUp()
        ...
        # test neighbor creation
        self.neighbor_entry1 = sai_thrift_neighbor_entry_t(
            self.switch_id, self.port0_rif, sai_ipaddress(ip_addr1))
        sai_thrift_create_neighbor_entry(
            self.client, self.neighbor_entry1, dst_mac_address=dmac1)
        self.neighbor_entry2 = sai_thrift_neighbor_entry_t(
            self.switch_id, self.port1_rif, sai_ipaddress(ip_addr2))
        sai_thrift_create_neighbor_entry(
            self.client, self.neighbor_entry2, dst_mac_address=dmac2)
        self.nhop1 = sai_thrift_create_next_hop(
            self.client,
    ...
    def tearDown(self):
        ...
        super(L3IPv6EcmpHostTwoPortsHelper, self).tearDown()


class L3IPv6EcmpHostTwoPortsTest(L3IPv6EcmpHostTwoPortsHelper):
    """
    IPv6 ECMP tests with all members as regular L3 port RIFs
    """
    def setUp(self):
        super(L3IPv6EcmpHostTwoPortsTest, self).setUp()

    def runTest(self):
        print("L3IPv6EcmpHostTwoPortsTest")
        # send the test packet(s)
        pkt1 = simple_tcpv6_packet(eth_dst=ROUTER_MAC,
                                  eth_src='00:22:22:22:22:22',
                                  ipv6_dst='5000:1:1:0:0:0:0:1',
        ...
        exp_pkt2 = simple_tcpv6_packet(eth_dst='00:11:22:33:44:56',
                                       ...
                                       ipv6_hlim=63)
        print("Sending packet port %d -> port %d"
              "(2000:1:1:0:0:0:0:1 -> 5000:1:1:0:0:0:0:1 [id = 101])" % (
                  self.dev_port1, self.dev_port0))
        send_packet(self, self.dev_port1, pkt1)
        verify_packet(
            self, exp_pkt1, self.dev_port0)
        ...
        print("Sending packet port %d -> port %d"
              "(2000:1:1:0:0:0:0:1 -> 5000:1:1:0:0:0:0:2 [id = 101])" % (
                  self.dev_port0, self.dev_port1))
        send_packet(self, self.dev_port0, pkt2)
        verify_packet(
            self, exp_pkt2, self.dev_port1)
    ...
    def tearDown(self):
        super(L3IPv6EcmpHostTwoPortsTest, self).tearDown()


class L3IPv6EcmpRoutingTwoPortsTest(L3IPv6EcmpHostTwoPortsHelper):
    """
    IPv6 ECMP routing test with random host
    """
    def setUp(self):
        super(L3IPv6EcmpRoutingTwoPortsTest, self).setUp()

    def runTest(self):
        print("L3IPv6EcmpRoutingTwoPortsTest")
        for n_packet in range(4):
            host_ip = f'5000:1:1:0:0:0:0:{random.randint(3, 1000)}'
            pkt = simple_tcpv6_packet(eth_dst=ROUTER_MAC,
                                        eth_src='00:22:22:22:22:22',
    ...
    def tearDown(self):
        super(L3IPv6EcmpRoutingTwoPortsTest, self).tearDown()
